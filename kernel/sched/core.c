...
int sched_fork(unsigned long clone_flags, struct task_struct *p)
{
	 ...
	 */
	p->state = TASK_NEW;  ///保证进程不会执行，不响应外部信号，还没加入调度器
...
static void __sched notrace __schedule(bool preempt)
{
	...
	schedule_debug(prev, preempt); ///判断是否是atomic上下文，若是则为bug
	 ...
	 */
	prev_state = prev->state;
	if (!preempt && prev_state) {///判断为主动让出CPU
...
asmlinkage __visible void __sched preempt_schedule_irq(void)
{
	...
	do {
		preempt_disable();
		local_irq_enable();
		__schedule(true);  ///中断返回前，检查是否需要调度，这里是抢占调度，传入true
		local_irq_disable();
		sched_preempt_enable_no_resched();
	} while (need_resched());
